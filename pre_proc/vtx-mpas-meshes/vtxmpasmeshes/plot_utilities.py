...
def plot_mpas_darray_gtm(ds, vname, ax=None, outfile=None, **kwargs):
    ...
    for coord in ['Time', 'nVertLevels']:
        if coord in da.dims:
            if coord == 'Time':
                ...
                print('Selecting slice 2 for ' + coord + '.')
                da = da.isel({coord: 2})
    ...
    if 'nCells' in ds[vname].dims:
        #plot_cells_mpas(ds, vname, ax, **plot_kwargs)
        # GTM: test to plot 3D vars for t=0 and l=0
...
def plot_cells_mpas_3D(ds, vname, ax, da, **plot_kwargs):
    for i, cell in enumerate(ds['nCells'].values):
        #value = ds[vname].sel(nCells=cell)
