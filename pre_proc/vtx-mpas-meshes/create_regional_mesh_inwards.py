import os
import pandas as pd
from vtxmpasmeshes.mesh_generator_inwards import full_generation_process_gtm
import argparse
...
# Add file name to input_file.txt
print (args.exp_dir)
try:
    # Open the file in append mode
    with open(f'{args.exp_dir}/input_file.txt', 'a') as file:
        # Append the line
        file.write(f'file_name={name}.region' + '\n')
    print(f'{name}.region added to input_file.txt.')
    os.system(f'cp {args.exp_dir}/input_file.txt {OUTPUT_DIR}')
    print(f'input_file.txt copied to {OUTPUT_DIR}')
